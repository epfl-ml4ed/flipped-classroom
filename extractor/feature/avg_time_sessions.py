...
class AvgTimeSessions(Feature):
    ...
    def compute(self):
        if len(self.data.index) == 0:
            return 0.0
        return np.mean(get_sessions(self.data.sort_values(by='date'), self.settings['max_session_length'], self.settings['min_actions'])['duration'])
